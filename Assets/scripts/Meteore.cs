...
public class Meteore : DestructionObject
{
    ...
    public override void Destroy(Tilemap tilemap, WaterSystem waterSystem, HeightManager heightManager, Vector2 pos)
    {
        StartCoroutine(DestroyAfterDelay(tilemap, waterSystem, heightManager, pos));
    }

    private IEnumerator DestroyAfterDelay(Tilemap tilemap, WaterSystem waterSystem, HeightManager heightManager, Vector2 pos)
    {
        ...
        int cellRadiusX = Mathf.CeilToInt(meteoriteSize * 1.5f / tilemap.cellSize.x);
        int cellRadiusY = Mathf.CeilToInt(meteoriteSize * 1.5f / tilemap.cellSize.y);
